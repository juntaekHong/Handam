...
const Calculation = props => {
    ...
    return (
      <View style={{ flexDirection: "row", marginTop: widthPercentageToDP(7) }}>
        ...
        <MenuTriggerBtn
          ...
          text={
            data.score.indexOf(classData[props.index].grade) != -1
              ? data.grade[data.score.indexOf(classData[props.index].grade)]
              : "선택"
          }
        />
      </View>
  ...
  const GradeList = () => {
    const index =
      data.score.indexOf(classData[selectorModal.index].grade) != -1
        ? data.score.indexOf(classData[selectorModal.index].grade)
        : null;
        ...
        {data.grade.map((item, index) => {
          return (
            <View>
              ...
              {data.grade.length !== index + 1 ? <GradeLineView /> : null}
            </View>
          );
        })}
      </View>
