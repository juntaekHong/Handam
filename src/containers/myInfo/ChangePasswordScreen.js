...
import {SECText, PassStandText, PassCheckText, CustomModalBlackText} from "../../components/myInfo/Text";
...
class ChangePasswordScreen extends React.Component {
    ...
    renderSubmit = () => {
        if (this.state.nowPass != '' && this.state.changePass != '' && this.state.passCheck != '') {
            return (
                <TouchableOpacity
                    style={styles.submit}
                    onPress={async () => {
                        ...
                        let comparePass = checkPassCompare(this.state.changePass, this.state.passCheck);
                        let compareChange = checkPassCompare(this.state.nowPass, this.state.passCheck);

                        if(check && checkPass(this.state.passCheck) && comparePass && !compareChange) {
    ...
    validCheck = () => {
        if(this.state.changePass != '' && checkPass(this.state.changePass) == true) {
            return <PassCheckText style={{ color: '#5ec133'}}>비밀번호 조건에 만족합니다.</PassCheckText>
        } else {
            return (
    ...
    compareCheck = () => {
        if(this.state.passCheck == '') {
            ...
            return (
                checkPassCompare(this.state.changePass, this.state.passCheck) == false ?
                    <PassCheckText>비밀번호가 일치하지않습니다.</PassCheckText>
                    :
                    <PassCheckText style={{ color: '#5ec133'}}>비밀번호가 일치합니다.</PassCheckText>
            )
        }
    };
        ...
        return (
            <SafeAreaView>
                <CustomModal
                    ...
                    footerHandler={async () => {
                        await MyInfoActions.changePassHandle(this.state.nowPass, this.state.changePass);
                ...
                <ScrollView>
                    ...
                    >
                        <PassStandText style={{marginBottom: widthPercentageToDP(17)}}>{'소중한 정보 보호를 위해\n현재 비밀번호를 확인해 주세요'}</PassStandText>
                        <TextInput
                            autoCapitalize={"none"}
                            onChangeText={(nowPass) => {this.setState({nowPass})}}
                        ...
                        <TextInput
                            ...
                            style={this.state.changePass == '' ? styles.inputBorder : styles.notInputBorder}
                            underlineColorAndroid="transparent"
                            secureTextEntry={true}
                            placeholderTextColor={"#9e9e9e"}
                            selectionColor={"#24a0fa"}
                        ...
                        <TextInput
                            ...
                            style={this.state.passCheck == '' ? styles.inputBorder : styles.notInputBorder}
                            underlineColorAndroid="transparent"
                            secureTextEntry={true}
                            placeholderTextColor={"#9e9e9e"}
                            selectionColor={"#24a0fa"}
